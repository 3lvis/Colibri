...
class WrapperView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.webView.addObserver(self, forKeyPath: WrapperView.URLKeyPath, options: NSKeyValueObservingOptions.new, context: nil)
    }
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == WrapperView.URLKeyPath {
            self.viewDelegate?.wrapperView(self, didChangeURL: self.webView.url?.absoluteString ?? "")
        }
    }
